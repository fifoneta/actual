...
def match_saturation_character(audio: np.ndarray, sr: int,
    ...
    audio_out = harmonic_saturation(audio, drive=drive, mode=mode, mix=1.0)
...
# ─── Peso perceptual Fletcher-Munson para EQ matching ────────────────────────
# El oído humano NO es plano — somos ~10 dB más sensibles a 3-4 kHz que a
# 100 Hz o 15 kHz (curvas de igual sonoridad ISO 226). Si el matching trata
# todas las bandas con el mismo peso, una corrección de +2 dB en 3.5 kHz
# suena mucho más agresiva que +2 dB en 100 Hz. El peso FM invierte esa
# sensibilidad: amplifica la importancia de las bandas donde el oído es menos
# sensible (sub-graves, aire) y reduce la de las bandas donde ya es muy
# sensible (presencia). El resultado es un matching que "suena igual" en vez
# de "mide igual".
#
# Modelo: curva A-weighting simplificada + bump de presencia inversa.
# No es la ISO 226 completa (requeriría interpolación de tablas por nivel SPL)
# sino una aproximación continua suficiente para ponderar un EQ de matching.

def _fletcher_munson_weight(freqs_hz: np.ndarray,
                             reference_level_phon: float = 70.0) -> np.ndarray:
    """Devuelve pesos de importancia perceptual para cada frecuencia.

    A 70 phon (nivel de escucha típico de mastering):
      - Sub-graves (20-80 Hz):   peso alto  → el oído los subestima, hay que
                                              darles más importancia en el match
      - Medios-bajos (300-1kHz): peso medio → respuesta bastante lineal
      - Presencia (2-5 kHz):     peso bajo  → el oído ya los amplifica sólo,
                                              el match no necesita ser agresivo
      - Aire (10-20 kHz):        peso medio-alto → roll-off natural del oído

    Retorna array normalizado (media = 1.0) para usar como multiplicador
    de la curva de corrección.
    """
    f = np.asarray(freqs_hz, dtype=np.float64)
    f_safe = np.clip(f, 20.0, 20000.0)

    # A-weighting (IEC 61672) — mide la sensibilidad relativa del oído
    f2 = f_safe ** 2
    f4 = f2 ** 2
    Ra = (12194.0**2 * f4) / (
        (f2 + 20.6**2) *
    ...
    # A(f) en dB — positivo donde el oído es MÁS sensible
    A_db = 20.0 * np.log10(Ra + 1e-12) + 2.0  # +2 dB normalization factor

    # Invertimos: donde el oído es más sensible (A alto) le damos MENOS peso
    # para que el match no sobrecompense esas frecuencias
    weight = 1.0 - np.clip(A_db / 60.0, -0.5, 0.5)

    # Boost adicional en sub-graves para nivel 70 phon
    # (a ese nivel la curva de igual sonoridad tiene un bump ~+8 dB en 60Hz)
    sub_boost = np.where(f_safe < 120.0,
                          0.3 * np.exp(-((np.log10(f_safe) - np.log10(60.0)) / 0.25) ** 2),
                          0.0)
    weight = weight + sub_boost

    # Normalizar a media = 1.0
    weight = weight / (np.mean(weight) + 1e-9)
...
def compute_reference_eq_curve(src_bands_db: list, ref_bands_db: list, freqs_hz: list,
                                ...
                                smooth_window: int = 3,
                                blend: float = 0.75,
                                use_perceptual_weight: bool = True) -> list:
    ...
    # ── Peso perceptual Fletcher-Munson ───────────────────────────────────────
    # Pondera la corrección según la sensibilidad del oído: amplifica la
    # importancia de sub-graves y aire (donde el oído es menos sensible) y
    # reduce la de presencia (donde el oído ya amplifica naturalmente).
    # El resultado es un match que suena más natural a nivel de escucha típico.
    if use_perceptual_weight and len(freqs_hz) > 1:
        fm_weight = _fletcher_munson_weight(np.array(freqs_hz, dtype=np.float64))
        diff = diff * fm_weight
        diff = diff - np.mean(diff)  # re-centrar tras la ponderación
...
def process_audio_with_reference(
    ...
    # ── 2. EQ de matching (FIR de fase lineal) ───────────────────────────
    # Modo M/S (default): calcula curvas INDEPENDIENTES para Mid y Side.
    #   - Mid: controla kick, bajo, voz — el cuerpo mono de la mezcla.
    #   - Side: controla reverb, width, aire — la diferencia L-R.
    # Las referencias profesionales tienen un balance tonal distinto en M vs S
    # (más sub en Mid, más aire en Side). Matchear en M/S captura eso.
    # Modo legacy (ms_eq_matching=False): un único FIR aplicado a L y R igual.
    _report(progress_cb, 20, "Calculando y aplicando EQ de matching (FIR M/S)")
    audio = eq_high_pass(audio, sr, cutoff_hz=hp_cutoff)
    if ms_eq_matching and audio.ndim == 2 and audio.shape[0] == 2:
        ...
        _eq_curve_for_report = curve_mid  # reportamos la curva Mid (más representativa)
    else:
    ...
    return {
        ...
        "reference_match": {
            ...
            "ms_eq_matching":          ms_eq_matching and audio.ndim == 2,
